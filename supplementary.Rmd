...
This webpage provides a collection of supplementary plots and datasets for the article 'Reshaping musical consonance with timbral manipulations and massive online experiments' by Raja Marjieh, Peter M. C. Harrison, Harin Lee, Fotini Deligiannaki, and Nori Jacoby. Click on the different tabs to navigate through the different plots and datasets.
...
spec <- list(
  ...
  "Study 2" = list(
    ...
    "2B: Bonang dyads" = DyadSupplementary(
      "4 - Bonang dyads",
  ...
  "Study 6" = list(
    "6(i): Harmonic dyads" = DyadSupplementary(
  ...
  "Study 7" = RawDataOnly(
    info = paste0(
      "This study investigated the joint manipulation of timbre and pitch interval with GSP.",
      " There are no supplementary analyses but you can download the data here:"
    ),
    raw_data_path = "input/data-csv/GSP/tmb_int_gsp_mixintm.csv",
    processed_data_path = "output/plots/080-timbre-and-consonance-gsp-pitch-interval-profiles.csv"
  )
)
...
triad_model_panel <- function(category, model_label, path) {
  tabPanel(
    model_label,
    embed_image(
      file.path(path, "models", category, paste0(model_label, ".png"))
    )
  )
}
